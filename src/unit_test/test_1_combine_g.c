...
int main(int argc, char *argv[]) {
  ...
  for (int i=0; i< no_v; i++){
    ...
    if ((i+1)==no_v){
       if (add_e(&G1,i, i, 0, 2, 17, 17)==-1){return log_error(output_str);}
       if (add_e(&G2,i, i, 0, 2, 17, 17)==-1){return log_error(output_str);}       
    } else {
      if (add_e(&G1,i, i, (i+1), 2, 17, 17)==-1){return log_error(output_str);}
      if (add_e(&G2,i, i, (i+1), 2, 17, 17)==-1){return log_error(output_str);}
    } // end if ((i+1)==no_v)   
  }
  /**
