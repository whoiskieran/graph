...
/**
* Test get_e_id to ensure it returns the correct id
**/
...
int main(int argc, char *argv[]) {
  ...
  **/
  ...
  for (int i=0; i< no_e; i++){
    sprintf(msg,"set_e_id returned error for edge %d ",i);
    if (set_e_id(&G,i,i) ==-1){return log_error(msg);} 
    sprintf(msg,"get_e_id returned error for edge %d ",i);
    if(get_e_id(&G,i)==-1){return log_error(msg);}
  }
