...
/**
* Try pasing down an invalid endge of -1 to the get_bk_weight function.
* It sould retrun -1
**/

int main(int argc, char *argv[]) {
  graph G;
  int no_e =0; int no_v=0; int w=18;
  char IfileName[1024];
  char OfileName[1024];
  int degree =0;
  int sum_degree =0;
  ...
  **/
  ...
  for (int i=0; i< no_e; i++){
    sprintf(msg,"set_bk_weight returned error for edge %d ",i);
    if (set_bk_weight(&G,i,w)==-1){return log_error(msg);}
    sprintf(msg,"get_bk_weight returned error for edge %d weight=%ld",i,get_bk_weight(&G,i));
    if (get_bk_weight(&G,-1) !=-1){return log_error(msg);}
  }
