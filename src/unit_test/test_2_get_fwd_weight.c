...
/**
* Pass down an invalid edge of -1 to get_fwd_weight
...
int main(int argc, char *argv[]) {
  ...
  **/
  ...
  for (int i=0; i< no_e; i++){
    sprintf(msg,"set_fwd_weight returned error for edge %d ",i);
    if (set_fwd_weight(&G,i,w)==-1){return log_error(msg);}
    sprintf(msg,"get_fwd_weight returned error for edge %d weight=%ld",i,get_fwd_weight(&G,i));
    if (get_fwd_weight(&G,-1) !=-1){return log_error(msg);}
  }
