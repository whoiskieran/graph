...
long get_no_regions(graph *G){return (G->no_e - G->no_v)+2;}

/**
* rotate_v 
* Pass down the steps you want to advance the verticies by.
* pass down the grpah.
* If a vertix is 3 and there are only 3 verticies it will be rotated back to 0.
* This behaves like a circular menu when you press the up arrow at the top you go back to the bottom.
* 
* With the exception of the 5 known Vertix Trnasitave Non Hameltonian groah
* Any N-regular graph will have consistant adjacency when the vertix are rotated.
*
* For example if a triangular graph which is 2 regular has its nodes rotated it will
* Keep its consistancy V1 will be always adjacent to V0 and V2
* I disvovered this when numbering vertices on the VTNH graphs.
* someone has probably discovered this before me.
**/

long rotate_v(graph *G, long steps){
  long *edge_list;
  long e_size, i,j,k;
  
  for (i=0;i < steps;i++){
    for (j=0; j< G->no_v; j++){
      // Find the edges that start or end at vertix j
      edge_list = find_edges(G,j,0,2);
      e_size = edge_list[0];
      // increment the vertix by 1
      // point the edge start 
      for (k=0; k < e_size; k++){
      }
      free(edge_list);
    } // end for j
